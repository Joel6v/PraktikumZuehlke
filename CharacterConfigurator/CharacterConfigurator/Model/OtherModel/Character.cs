...
namespace CharacterConfigurator.Model
{
    public class Character : IBaseModel<Character>, IBaseModelVariable<Character>
    {
        ...
        {
            return new List<string>() { $"'{Name}'", $"'{TimeStamp.ToString(DataHandler.FormtDb)}'", $"{User.Id}", $"{Race.Id}", $"{Headgear.Id}", $"{Chest.Id}", $"{Gloves.Id}", $"{Legs.Id}", $"{Weapon.Id}", $"{Consumable.Id}", $"{(int)Sex}"};
        }
